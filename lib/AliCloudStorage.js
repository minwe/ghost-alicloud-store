...
class AliCloudStorage extends StorageBase {
  constructor(options) {
    ...
    ['endpoint', 'cname', 'timeout', 'secure'].forEach(key => {
      if (key in options) {
    ...
    utils.utils.log('ossOpts', ossOpts);
  ...
  async save(file, targetDir) {
    ...
    try {
      ...
      utils.log('save result -> name: ', result.name);
      utils.log('save result -> url: ', result.url);

      if (utils.isOk(result)) {
        const endpoint = this.options.endpointProxy;
        return endpoint ? `${endpoint}/${name}` : result.url;
      }
